...
class Service_category extends MY_Controller {
    ...
    public function index() {
        ...
        $config['base_url'] = base_url('admincp/service_category/index');
        ...
        $this->data['temp'] = 'service_category/index';
    ...
    public function detail($id = 0) {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($pid) {
                    redirect(base_url('admincp/service_category?id=&vn_name=&pid=' . $pid));
                } else {
                    redirect(base_url() . 'admincp/service_category/index/');
                }
            }
        }
        ...
        $this->data['temp'] = 'service_category/detail';
        $this->load->view('admin/main', $this->data);
    }
        ...
        switch ($action) {
            case 'del':
                ...
                if ($check_catalog) {
                    ...
                    $check_service_sub = $this->articles_m->check_exists('cid in (' . $id_object . ')');
                    $input = 'id in(' . $id_object . ')';
                    //Check danh mục có tồn tại sản phẩm k
                    if ($check_service_sub) {
                    ...
                    $check_service = $this->articles_m->check_exists('cid = ' . $id . '');
            ...
            case 'del_all':
                ...
                if ($check_catalog_all) {
                    ...
                    $check_service_sub = $this->articles_m->check_exists('cid in (' . $id_object_all . ')');
                    ...
                    $check_service_all = $this->articles_m->check_exists('cid in (' . $array_id . ')');
                    //Check xem danh muc co san pham k
                    if ($check_service_all) {
                        //Xóa toàn bộ sản phẩm của danh mục đó xong xóa danh mục
                        $input = 'cid in (' . $id . ')';
                        if ($this->articles_m->update_rule($input, array('status' => 3))) {
                            if ($this->category->update_rule($where_in, array('status' => 3))) {
...
//                    }
        ...
        redirect(base_url('admincp/service_category'));
    }
